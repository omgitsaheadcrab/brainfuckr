...
(Try it both with file input a file consisting only of one blank line and with
keyboard input ie hit return and then ctrld (Unix) or ctrlz (Windows))
It should give two lines of output; the two lines should be identical and
should be lined up one over the other
If that doesn't happen ten is not coming through as newline on output
...
LA means newline input is working fine and EOF translates as negative 1
